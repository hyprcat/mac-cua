...
/// Minimal, dependency-free SHA-256 so the version-hash header matches the
/// Python `hashlib.sha256(...).hexdigest()[:8]` byte-for-byte on Linux (where
/// Foundation ships no digest API). Not performance-critical — used once.
enum SHA256Lite {
    static func hexDigest(_ string: String) -> String {
        let digest = hash(Array(string.utf8))
